...
def dice_coefficient(probs, targets, smooth=1.0):
    """
    Calculates the Sørensen–Dice coefficient.

    Args:
        probs: (N, C, H, W) tensor of probabilities (output of softmax/sigmoid)
        targets: (N, H, W) tensor of long integers (ground truth labels)
        smooth: Smoothing factor to avoid division by zero.

    Returns:
        Dice coefficient per class (averaged over batch), shape (C,)
        or mean Dice coefficient across classes (scalar) depending on implementation needs.
    """
    ...
    probs_flat = probs.view(probs.shape[0], probs.shape[1], -1) # (N, C, H*W)
    targets_flat = targets_one_hot.view(targets_one_hot.shape[0], targets_one_hot.shape[1], -1) # (N, C, H*W)

    intersection = torch.sum(probs_flat * targets_flat, dim=2) # (N, C)
    cardinality = torch.sum(probs_flat + targets_flat, dim=2)   # (N, C)

    dice_score = (2. * intersection + smooth) / (cardinality + smooth) # (N, C)

    # Average score per class over the batch
    dice_score_per_class = dice_score.mean(dim=0) # (C,)

    # Return the mean Dice score across non-background classes, or all classes
    # Option 1: Mean over all classes
    # return dice_score_per_class.mean() 
    # Option 2: Mean over non-background classes (assuming class 0 is background)
    if num_classes > 1:
        return dice_score_per_class[1:].mean() # Average Dice for classes 1 to C-1
    else:
        return dice_score_per_class.mean() # Only one class, return its Dice
...
class DiceLoss(nn.Module):
    ...
    def forward(self, logits, targets):
        ...
        # Calculate Dice coefficient (mean over non-background classes)
        dice = dice_coefficient(probs, targets, self.smooth)
        # Return Dice Loss
        return 1.0 - dice
...
class CombinedLoss(nn.Module):
    ...
    def __init__(self, weight_ce=0.5, weight_dice=0.5, dice_smooth=1.0):
        super().__init__()
        self.weight_ce = weight_ce
        self.weight_dice = weight_dice
        self.cross_entropy = nn.CrossEntropyLoss() # Standard CE
        self.dice_loss = DiceLoss(smooth=dice_smooth)
        print(f"Using CombinedLoss: CE weight={self.weight_ce}, Dice weight={self.weight_dice}")
    ...
    def forward(self, logits, targets):
        ...
        if torch.isnan(ce_loss):
            print("Warning: NaN detected in CrossEntropyLoss component.")
            ce_loss = 0 # Or handle differently
        if torch.isnan(dice_loss):
            print("Warning: NaN detected in DiceLoss component.")
            dice_loss = 0 # Or handle differently
            
        combined_loss = (self.weight_ce * ce_loss) + (self.weight_dice * dice_loss)
        
        # Add individual losses to track them (optional)
        # You might want to log these separately using a logger
        self._last_ce_loss = ce_loss.item()
        self._last_dice_loss = dice_loss.item()
